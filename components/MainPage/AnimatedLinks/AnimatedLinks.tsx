...
const AnimatedLinks: React.FC<{ links: { name: string; link: string; icon: string }[] }> = ({ links }) => {
    ...
    return (
        <div className="relative">
            <ul className="gap-1 md:gap-8 border border-[#FFFFFF30] px-2 py-1 md:px-6 md:py-2 rounded-full flex fixed md:relative right-1/2 md:right-0 translate-x-1/2 md:translate-x-0 bottom-6 md:bottom-0 z-10 w-11/12 md:w-fit bg-zinc-900 bg-opacity-80 backdrop-blur-2xl justify-between">
                {links.map((link, i) => (
                    <li
                        key={link.name}
                        id={"link-btn-" + i}
                        ...
                        <Link
                            ...
                            <span className="flex items-center justify-between md:hidden">{link.icon && <Icon icon={link.icon} className="w-5 h-5" />}</span>
                        </Link>
                    </li>
                ))}
